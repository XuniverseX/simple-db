...
//创建数据库表，同时将表对应的schema和layout存储到数据库里
//有两个特殊的表tblcat(tableName string, slotSize int), fldcat(tableName string, fieldName string, type FIELD_TYPE, length, offset)

const (
	MAX_NAME = 16 //tableName最大长度
)
