...
func (s *StatManager) GetStatInfo(tblName string, layout *rm.Layout, tx *tx.Transaction) *StatInfo {
	s.lock.Lock()
	defer s.lock.Unlock()
	s.numCalls += 1
...
func (s *StatManager) refreshStatistics(tx *tx.Transaction) {
...
func (s *StatManager) calcTableStats(tblName string, layout *rm.Layout, tx *tx.Transaction) *StatInfo {
	numRecs := 0
	numBlocks := 0
